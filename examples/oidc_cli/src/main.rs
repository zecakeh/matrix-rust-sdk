...
async fn use_auth_url(
    ...
    let (response, response_state) =
        match serde_html_form::from_str::<AuthResponse>(&response_query) {
            Ok(res) => (Ok(res.code), res.state),
            Err(_) => {
                // If the authorization didn't work, we should have gotten an error instead.
                let err = serde_html_form::from_str::<AuthError>(&response_query)?;
                (Err(err.error), err.state)
            }
        };
    ...
    // identify the session the was authorized.
...
/// The data returned by the provider after a successful authorization.
...
/// The data returned by the provider after an authorization error.
#[derive(Debug, Deserialize)]
struct AuthError {
    /// The error.
    #[serde(flatten)]
